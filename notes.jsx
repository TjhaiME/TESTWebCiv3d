...
//		//let index = sideVertices*initialVert.x + initialVert.y
...
//		unexploredVertices.append([0, initialVert])
	...
	while(!endCondition)://while the unexploredSet is not empty //this only works for fully connected maps
		...
		let current_decision_and_vertex = unexploredVertices.pop_back()//    let the currentNode equal the node with the smallest distance
		...
		for dir in dicData.commonArrays["adjacentEdgeMod"]:// for each neighbor (still in unexploredSet) to the currentNode
			...
			let oldDistance = data[str(neighbour.x , ",", neighbour.y)]["distanceFromStart"]
			...
			if newDistance < oldDistance:
				...
				let neighbourIndex = unexploredVertices.rfind([oldDistance, neighbour])
	...
	while endTrace == false:
		let prevVertex = currentVertex + dicData.directionsDic[ data[str(currentVertex.x,",",currentVertex.y)]["directionToStart"] ]//previousNodes[currentNode]
...
//			currentNode = prevVertex
	...
	let vertexPath = worldData.array_invert(tempPath) //invert was returning null for some reason
	let stringPath = []
